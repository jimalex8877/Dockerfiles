...
if [ "$BOOTSTRAP_SERVERS" ]; then
	sed -i "s|bootstrap.servers=localhost:9092|bootstrap.servers=$BOOTSTRAP_SERVERS|" "$KAFKA_HOME"/config/connect-distributed.properties;
	sed -i "s|bootstrap.servers=localhost:9092|bootstrap.servers=$BOOTSTRAP_SERVERS|" "$KAFKA_HOME"/config/connect-standalone.properties;
	sed -i "s|bootstrap.servers=localhost:9092|bootstrap.servers=$BOOTSTRAP_SERVERS|" "$KAFKA_HOME"/config/producer.properties;
	sed -i "s|bootstrap.servers=localhost:9092|bootstrap.servers=$BOOTSTRAP_SERVERS|" "$KAFKA_HOME"/config/consumer.properties;
fi
